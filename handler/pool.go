...
func (a *App) GetPoolInfoList(userId int64) (poolInfoList []PoolInfo, err error) {
	var user model.User
	_, err = db.Engine.ID(userId).Get(&user)
	if err != nil {
		return nil, err
	...
	if err != nil {
		log.Error().Err(err).Msg("")
		return nil, errors.New("error occurred when get lang map")
	}

	itemDataMap, err := service.GetItemDataMap(user.GameDataDir, user.GameServer)
	if err != nil {
		log.Error().Err(err).Msg("")
		return nil, errors.New("error occurred when get item map")
	}

	dollIconMap, err := service.GetDollIconMap(user.GameDataDir, user.GameServer)
	if err != nil {
		log.Error().Err(err).Msg("")
		return nil, errors.New("error occurred when get doll icon map")
	}

	weaponIconMap, err := service.GetWeaponIconMap(user.GameDataDir, user.GameServer)
	if err != nil {
		log.Error().Err(err).Msg("")
		return nil, errors.New("error occurred when get weapon icon map")
	}

	poolTypeList, err := service.GetGachaPoolTypeList(user.GameDataDir, user.GameServer)
	if err != nil {
		log.Error().Err(err).Msg("")
		return nil, errors.New("error occurred when get gacha pool type list")
	}
	...
	if err != nil {
		log.Error().Err(err).Msg("")
		return nil, errors.New("error occurred when get gacha pool map")
	}

	for _, poolType := range poolTypeList {
		var recordList []model.Record
		err = db.Engine.OrderBy("id").Find(&recordList, &model.Record{UserId: userId, PoolType: poolType})
		if err != nil {
			log.Error().Err(err).Msg("")
			return nil, errors.New("error occurred when get user record list by poolType from db")
		}
		if len(recordList) > 0 {
			...
			for _, record := range recordList {
				...
				if item, hasItemData := itemDataMap[record.ItemId]; hasItemData {
						...
						if text, hasLangData := langDataMap[item.Name.Id]; hasLangData {
							...
							if item.Type == 10 {
								if i, hasIcon := dollIconMap[item.Id]; hasIcon {
									...
									return nil, errors.Errorf("error occurred when get doll icon by id %d", item.Id)
								}
							} else if item.Type == 20 {
								if i, hasIcon := weaponIconMap[item.Id]; hasIcon {
									icon = i
								} else {
									return nil, errors.Errorf("error occurred when get weapon icon by id %d", item.Id)
								}
							}
							...
							return nil, errors.Errorf("error occurred when get item name by id:%d", record.ItemId)
						}
						count = 0
					...
					return nil, errors.Errorf("error occurred when get item data by id:%d", record.ItemId)
				}
			}
